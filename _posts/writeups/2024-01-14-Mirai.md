...
> **Info**: Mirai’s go-to attack was to brute force common default passwords. In fact, mirai-botnet.txt was added to SecLists in November 2017. 
...
This will give as that pi can run any command as root without password, so we just need to run `sudo su` to get a root shell and read the root flag.
